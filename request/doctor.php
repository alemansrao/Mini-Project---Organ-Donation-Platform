...
if (isset($_POST['submit'])) {
    ...
    $reqID = $userid . "_Doctor";
    $insertQuery = "insert into requests_table values('$reqID','$userid','$email','Doctor','$name','$enteredAddress','$pincode',$enteredPhone,'" . date("Y-m-j") . "');";
    if (mysqli_query($conn, $insertQuery)) {
        $msg = "Name : $name\nAddress : $address\nPincode : $pincode\nContact No : $phone";
        $mailSent = mail(getAllEmail("Doctor"), "Doctor Needed Urgent", $msg);
        if ($mailSent) {
            output("All the doctors has been alerted by email");
        }
        else{
            output("Mail Not Sent");
        }
    } else if (mysqli_errno($conn) == 1062) {
        output("You Have already requested!<br> Please wait for Doctor's response");
    } else {
    ...
    <div class="container">
        <form action="" method="post">
            ...
            <div class="input-group mb-3 ">
                ...
                <input type="text" class="form-control" placeholder="eye" aria-label="Username" value="&#129658;Doctor" disabled>
            </div>
            <div class="input-group mb-3 ">
                <div class="input-group-prepend">
