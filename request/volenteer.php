...
if (isset($_POST['submit'])) {
    ...
    $reqID = $userid . "_Volunteer";
    $insertQuery = "insert into requests_table values('$reqID','$userid','$email','Volunteer','$name','$enteredAddress','$pincode',$enteredPhone,'" . date("Y-m-j") . "');";
    // echo $insertQuery;
    if (mysqli_query($conn, $insertQuery)) {
        $msg = "Name : $name\nAddress : $address\nPincode : $pincode\nContact No : $phone";
        $mailSent = mail(getAllEmail("Volunteer"), "Volunteer Needed Urgent", $msg);
        if ($mailSent) {
            output("All the volunteers has been alerted by email");
        }
        else{
            output("Mail Not Sent");
        }
    } else if (mysqli_errno($conn) == 1062) {
        output("You Have already requested!<br> Please wait for Volunteer's response");
    } else {
    ...
    <div class="container">
        <form action="" method="post">
            ...
            <div class="input-group mb-3 ">
                ...
                <input type="text" class="form-control" placeholder="eye" aria-label="Username" value="&#127384;Volunteer" disabled>
            </div>
            <div class="input-group mb-3 ">
                <div class="input-group-prepend">
