...
if (isset($_POST['submit'])) {
    ...
    $reqID = $userid . "_Blood";
    $insertQuery = "insert into requests_table values('$reqID','$userid','$email','Blood','$name','$enteredAddress','$pincode',$enteredPhone,'" . date("Y-m-j") . "');";
    if (mysqli_query($conn, $insertQuery)) {
        $msg = "Name : $name\nAddress : $address\nPincode : $pincode\nContact No : $phone";
        $mailSent = mail(getAllEmail("Blood"), "Blood Donor Needed Urgent", $msg);
        if ($mailSent) {
            output("All the Blood donors has been alerted by email");
        } else {
            output("Mail Not Sent");
        }
    } else if (mysqli_errno($conn) == 1062) {
        output("You Have already requested!<br> Please wait for Donor response");
    } else {
...
<head>
    ...
    <title>Look for Eye donors</title>
    ...
    <div class="container">
        <form action="" method="post">
            ...
            <div class="input-group mb-3 ">
                ...
                <input type="text" class="form-control" placeholder="eye" aria-label="Username" value="&#129656;Blood" disabled>
            </div>
            <div class="input-group mb-3 ">
                <div class="input-group-prepend">
